...
def create_hooks_for_layer_mode(
    ...
    def replace_output_hook(mlp_output: torch.Tensor, hook) -> torch.Tensor:
        ...
        if mode == "no_delta":
            ...
            recon_norm = base_recon_norm + alpha * delta_recon_norm
...
def run_experiment(args):
    ...
    for L in layer_list:
        ...
        print(f"[Delta] Loading finetuned DeltaTranscoder for layer {L} ...")
        ...
        for subset_name, exs in subsets.items():
            ...
            for alpha in delta_alphas:
                ...
                print(f"[Scaled] Evaluating subset={subset_name}, layer={L}, alpha={alpha} ...")

                scaled_gen_log = os.path.join(
                    gen_dir, f"layer_{L}_{subset_name}_{cond_name}.jsonl"
                )
                if os.path.exists(scaled_gen_log):
                    os.remove(scaled_gen_log)

                scaled_stats = eval_subset_for_condition(
                    model=model,
                    tokenizer=tokenizer,
                    examples=exs,
                    fwd_hooks=scaled_hooks,
                    device=device,
                    condition=cond_name,
                    gen_log_path=scaled_gen_log,
                )

                sc_base_act, sc_delta_act = norm_act_metrics(scaled_act_metrics)
                ...
                rows.append(dict(
                    ...
                    percentile=args.percentile,
                    alpha=alpha,
                    mean_ce=scaled_stats["mean_ce"],
                    tool_call_rate=scaled_stats["tool_call_rate"],
                    valid_tool_format_rate=scaled_stats["valid_tool_format_rate"],
                    math_correct_rate=scaled_stats["math_correct_rate"],
                    mean_abs_base_acts=sc_base_act,
                    mean_abs_delta_acts=sc_delta_act,
                    n_examples=scaled_stats["n_examples"],
    ...
    csv_path = os.path.join(args.output_dir, f"exp2B_metrics_{tag}.csv")
    json_path = os.path.join(args.output_dir, f"exp2B_metrics_{tag}.json")
    os.makedirs(args.output_dir, exist_ok=True)
    ...
    # (subset, layer, alpha) -> row
    rows_by_sla: Dict[Tuple[str, int, float], Dict[str, Any]] = {}
    for r in rows:
        subset = r["subset"]
        layer = int(r["layer"])
    ...
    for r in rows:
        if r["condition"] == "raw":
            raw_by_subset[r["subset"]] = r

    metrics_to_plot = [
        "mean_ce",
        "tool_call_rate",
        "valid_tool_format_rate",
        "math_correct_rate",
    ]
    metric_titles = {
        "mean_ce": "Mean Cross-Entropy",
    ...
    for subset_name in subsets_names:
        ...
        alphas = sorted({
            ...
            and isinstance(r.get("alpha", None), (int, float))
            and not math.isnan(float(r["alpha"]))
        })
        if not alphas:
            continue
        ...
        for idx, metric_name in enumerate(metrics_to_plot):
            ...
            for L in layers_sorted:
                ...
                ys: List[float] = []
                for a in alphas:
                    row = rows_by_sla.get((subset_name, L, a), None)
                    if row is None:
                        continue
                    xs.append(a)
                    ys.append(row.get(metric_name, float("nan")))
                if not xs or all(math.isnan(v) for v in ys):
                    continue
            ...
            if not any_line and (
                raw_row is None
                or math.isnan(raw_row.get(metric_name, float("nan")))
            ):
                ax.set_visible(False)
                continue

            ax.set_title(metric_titles.get(metric_name, metric_name))
            ax.set_xlabel("alpha (delta scaling)")
            if idx == 0:
                ax.set_ylabel(metric_titles.get(metric_name, metric_name))
                ax.legend()
            ax.grid(True, alpha=0.3)

        fig.suptitle(
            f"Exp2B metrics vs alpha (subset={subset_name}, "
            f"d_new={args.d_new}, pctl={args.percentile})"
        )
        fig_path = os.path.join(
            graphs_dir,
            f"exp2B_metrics_vs_alpha_{subset_name}_{tag}.png",
        )
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(fig_path)
        plt.close()
        print(f"[Save] 2B metrics-vs-alpha figure for subset={subset_name} -> {fig_path}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Experiment 2B: delta scaling on selected layers"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of delta scaling factors (e.g. '0.0,0.25,0.5,0.75,1.0')",
    )
...
'''
python3 exp2B_delta_scaling.py \
