...
def find_delta_checkpoint(
    ...
    """
    Automatically find best_model.pt under delta_root based on layer, d_new, percentile.
    The directory is assumed to be roughly like:
...
def load_jsonl_by_label(
    ...
    """
    Load JSONL file, each line must contain at least: input, target, label
    Split into subsets based on label: tool, copy, tool_ood
    """
...
def create_hooks_for_layer_mode(
    ...
    """
    Create hooks for a single layer, using fused SAE's base+delta:
      - mode='full'    : reconstruction = base + delta
      - mode='no_delta': reconstruction = base (but still accumulate delta activations)
    Also accumulate:
      - mean_abs_base_acts, mean_abs_delta_acts
    Accumulate into metrics dict (called once per subset during eval).
    """
    ...
    def replace_output_hook(mlp_output: torch.Tensor, hook) -> torch.Tensor:
        ...
        # Manually compute base / delta acts & recon
        ...
        # Accumulate activation magnitudes
        with torch.no_grad():
...
# ================================================================
# 4. Tool Call Parsing & Evaluation: CE + Tool Call Rate + Math Accuracy
# ================================================================
...
def extract_expr_from_tool_call(text: str) -> Optional[str]:
    """
    Extract the expr inside <tool_call>calculator(expr)</tool_call> from the text.
...
def safe_eval_expr(expr: str) -> Optional[float]:
    """
    Safely evaluate an expression like '123+45*6', allowing only 0-9 + - * / and whitespace.
    Return None if invalid or eval fails.
    """
...
def generate_with_hooks(
    ...
    """Given a prompt, perform greedy generation with optional hooks, returning the newly generated text."""
    with torch.no_grad():
...
def eval_subset_for_condition(
    ...
    - The other three metrics are based on actual generation
    - If gen_log_path is not None, write each sample's generation to that JSONL file
    """
    total_loss = 0.0
    total_tokens = 0
    n_examples = 0
    ...
    valid_tool_count = 0
    math_correct = 0
    math_total = 0  # Number of samples with gold expressions

    for ex in examples:
        prompt = str(ex["input"])
        target = str(ex["target"])

        # Encoding: encode prompt and target separately, then concatenate
        with torch.no_grad():
        ...
        # Only compute CE on the target interval:
        ...
        gen_text = generate_with_hooks(
            ...
            fwd_hooks=fwd_hooks,
            device=device,
            max_new_tokens=64,  # You can adjust according to your target length
        )
        ...
        # ---------- Math Accuracy ----------
        # Extract expr from gold target and pred_text, and compare if their numerical values match
...
# ================================================================
# 5. Main Experiment: per-layer, per-subset, per-condition
# ================================================================
...
def run_experiment(args):
    ...
    # ---------- 5.2 Load the validation set & split the subsets by label ----------
    ...
    # ---------- 5.3 Accumulate Results ----------
    rows: List[Dict[str, Any]] = []

    # ---------- 5.4 Raw baseline (layer-independent, computed once per subset) ----------
    ...
    # ---------- 5.5 For each layer: full / no_delta ----------
    for L in layer_list:
        print(f"\n========== Layer {L} ==========")

        # 5.5.1 Load base SAE & fused SAE
        ...
        # For each subset, for each condition: raw / full / no_delta
        for subset_name, exs in subsets.items():
            if not exs:
                continue
            ...
            # no_delta (base reconstruction only)
            ...
            # Write three rows: raw / full / no_delta
    ...
    # ---------- 5.7 Plotting ----------
    # First organize an index: by_key[(subset, layer, condition)] -> row
    by_key: Dict[Tuple[str, int, str], Dict[str, Any]] = {}
    for r in rows:
        key = (r["subset"], int(r["layer"]), r["condition"])
        # Later ones overwrite earlier ones (raw is the same for each layer and subset)
        by_key[key] = r
    ...
    # 5.7.2 Tool-related three metrics: RAW vs FULL vs NO-DELTA
    ...
    for subset_name in subsets_names:
        ...
        # Collect curves for each metric and each condition
        ...
        # Check if this subset is completely NaN across all three metrics (e.g., math_correct_rate for copy)
        all_vals = []
        for metric_name in metrics_to_plot:
            for c in conditions:
        ...
        # 5.7.3 Activation scale plots: only full condition, 1 figure with 3 subplots (one per subset)
    # x-axis: layer; y-axis: mean_abs_base_acts / mean_abs_delta_acts
    ...
    if subsets_names and layers_sorted_all:
        ...
        # If there is only one subset, axes is not an array, convert it to a list
