...
class DeltaTranscoder(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            # Ensure that the input data type matches the sub-module.
            x = x.to(next(self.base_transcoder.parameters()).dtype)
            base_recon, base_acts = self.base_transcoder(x)
            delta_recon, delta_acts = self.delta_features(x)
        
        return base_recon + delta_recon, base_acts, delta_acts
...
def create_hooks_for_observation(transcoders: Dict[int, nn.Module], context: Dict) -> List[Tuple[str, callable]]:
    fwd_hooks = []
    for layer_idx, transcoder_module in transcoders.items():
        ...
        def replace_output_hook(mlp_output, hook, tm=transcoder_module):
            ...
            recon, base_acts, delta_acts = tm(x_norm)
            recon = recon * norm_factor
            # Store activations in context for later analysis
            # Note: activations come from flattened input, so they are also flattened
            context[f'base_acts_{layer_index}'] = base_acts
            context[f'delta_acts_{layer_index}'] = delta_acts
...
def main(args):
    ...
    # 1. Load the "base" Transcoder used during fine-tuning
    ...
    # 2. Load your fine-tuned .pt file and reconstruct the full DeltaTranscoder
    ...
    fwd_hooks = create_hooks_for_observation(transcoders, context)
    print(f"Prepared hooks for finetuned transcoder at layer {args.sae_layer}.")

    # 3. Evaluation
    token_ids = tokenizer(args.prompt, return_tensors="pt")["input_ids"].to(device)

    # --- Perform forward pass with hooks ---
    ...
    # Retrieve activations from context
    base_acts = context.get(f'base_acts_{args.sae_layer}')
    delta_acts = context.get(f'delta_acts_{args.sae_layer}')
    
    if base_acts is None or delta_acts is None:
        print("\nERROR: Could not retrieve activations. Check hook logic.")
        return

    # Get activations for the last token (critical for next token prediction)
    # Activation tensor shape is (batch_size, num_tokens, feature_dim)
    last_token_base_acts = base_acts[0,-1]
    last_token_delta_acts = delta_acts[0,-1]
    
    # --- Compute metrics ---
    base_l0 = torch.count_nonzero(last_token_base_acts).item()
    delta_l0 = torch.count_nonzero(last_token_delta_acts).item()
    
    base_l1 = last_token_base_acts.abs().sum().item()
    delta_l1 = last_token_delta_acts.abs().sum().item()
    
    base_mean_act = base_l1 / max(1, base_l0) if base_l0 > 0 else 0
    delta_mean_act = delta_l1 / max(1, delta_l0) if delta_l0 > 0 else 0

    total_base_features = last_token_base_acts.shape[-1]
    total_delta_features = last_token_delta_acts.shape[-1]

    # --- Print report ---
    print(f"\nAnalysis for Layer {args.sae_layer} at the LAST token position:")
    print("-" * 70)
    print(f"| {'Metric':<25} | {'Base Features':<20} | {'Delta Features':<20} |")
    print(f"|{'-'*27}|{'-'*22}|{'-'*22}|")
    print(f"| {'L0 Norm (Active Features)':<25} | {base_l0:<20d} | {delta_l0:<20d} |")
    print(f"| {'Total Features':<25} | {total_base_features:<20d} | {total_delta_features:<20d} |")
    print(f"| {'Sparsity (L0 / Total)':<25} | {base_l0/total_base_features:<20.4f} | {delta_l0/total_delta_features:<20.4f} |")
    print(f"| {'L1 Norm (Sum of Acts)':<25} | {base_l1:<20.4f} | {delta_l1:<20.4f} |")
    print(f"| {'Mean Activation Value':<25} | {base_mean_act:<20.4f} | {delta_mean_act:<20.4f} |")
    print("-" * 70)

    print("\nNext Token Prediction:")
    probs = F.softmax(hooked_logits[0, -1], dim=-1)
    top_p, top_i = torch.topk(probs, 5)
    print("  " + ", ".join([f"{tokenizer.decode([i])} ({p:.2%})" for i,p in zip(top_i, top_p)]))
    print("="*60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="My name is Sarah.\n")
    parser.add_argument("--finetuned_path", type=str, required=True, help="Path to your finetuned best_model.pt file")
    # --- These parameters are needed to reconstruct the model structure ---
...
python3 validate_transcoder_Delta_activations.py --finetuned_path /usr/project/xtmp/qc62/train_4/dnew_128_pctl_0.5/layer_25_dnew_128_pctl_0.50/best_model.pt --sae_layer 25 --sae_selection_percentile 0.5 --d_new 128
'''